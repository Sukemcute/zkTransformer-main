...
from zkGPT_bindings import verify_integrity, secure_communication
import logger
...
class ConfidentialPrompting:
    def __init__(self, model_path, device):
        self.model = load_llm_model(model_path, device)
        logger.start_measure()

    def run(self, prompt):
        logger.log_measure("Prompt Received")
        ...
        # Run LLM inference
        results = self.model.run_inference(prompt)
...
if __name__ == "__main__":
    cp = ConfidentialPrompting("path/to/model", "cuda")
    result = cp.run("Your confidential prompt here")
